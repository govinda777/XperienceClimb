...
interface DestinationTemplateProps {
  destination: any;
  cmsEnabled: boolean;
  activeSiteFallback?: any;
}

export default async function DestinationTemplate({
  destination,
  cmsEnabled,
  activeSiteFallback,
  ...
  const activeSite = activeSiteFallback ?? {
    nextEventStartsAt: '',
  ...
  const communityCmsData =
    (visitedLocations && visitedLocations.length > 0) ||
    (safetyProcedures && safetyProcedures.length > 0) ||

...
import { mockLlmResponses } from '../mocks/llm-responses.mock.ts';
...
When('o utilizador envia a mensagem {string}', async (message: string) => {
  ...
  } else if (message.toLowerCase().includes('pastel') || message.toLowerCase().includes('receita')) {
...
Then('a resposta deve enfatizar que todos os procedimentos de segurança são efetuados exclusivamente pelos guias certificados no local', () => {
...
Then('o agente deve recusar responder de forma elegante e direcionar o utilizador de volta para temas de escalada e Pedra Bela', () => {
